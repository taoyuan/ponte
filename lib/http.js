...
function HTTP(opts, done) {
  ...
  this.server.listen(opts.port, opts.host, function(err) {
    ...
    logger.info({ host: opts.host, port: opts.port }, "server started");
  });
...
HTTP.prototype._subscribe = function(topic, req, res) {
  ...
  logger.info({topic: topic, remote: remote}, 'subscribe');
  ...
  function unsubscribe() {
    if (req in subscriptions) {
      logger.info({topic: topic, remote: remote}, 'unsubscribe for connection closed');
